...
with DAG(
    ...
    get_data = SimpleHttpOperator(
        ...
        data=json.dumps({
            "bucket_name": "gcp-datapipeline-389020-raw",
    ...
    PYSPARK_JOB = {
        ...
        "pyspark_job": {
            ...
            "args": [
                '--path_input',
                "gs://gcp-datapipeline-389020-raw/combustiveis-brasil/{{ dag_run.logical_date.strftime('%Y') }}/{{ '01' if dag_run.logical_date.month <= 6 else '02'}}/ca-{{ dag_run.logical_date.strftime('%Y') }}-{{ '01' if dag_run.logical_date.month <= 6 else '02'}}.csv",
                '--path_output',
                "gs://gcp-datapipeline-389020-curated/combustiveis-brasil/{{ dag_run.logical_date.strftime('%Y') }}/{{ '01' if dag_run.logical_date.month <= 6 else '02'}}/",
                '--file_format', 'parquet',
